...
html_files = sorted(glob.glob("downloads/saudi_gamer_page_*.html"))
...
for filename in html_files:
    ...
    if _HAS_BS4:
        ...
        for review in soup.select(".review-row"):
            ...
            # Find external 'Read full review' link (saudigamer review URL)
            a_external = review.find(
                "a", href=re.compile(r"^https?://[^\s]*saudigamer\.com/review/")
            )
        ...
        for block_match in block_pattern.finditer(html):
            ...
            ext_m = re.search(
                r'href="(https?://[^"]*saudigamer\.com/review/[^"]*)"', block
            )
...
# Save to JSON file
with open("saudi_gamer_games.json", "w", encoding="utf-8") as f:
    json.dump(games_dict, f, indent=2, ensure_ascii=False)

print(f"\nSaved to saudi_gamer_games.json")
