...
html_files = sorted(glob.glob("downloads/ign_page_*.html"))
...
for filename in html_files:
    ...
    if _HAS_BS4:
        ...
        for review in soup.select(".review-row"):
            ...
            # Find external 'Read full review' link (IGN review URL)
            # match subdomains like me.ign.com and paths that include '/review/' or '/reviews/'
            a_external = review.find(
                "a",
                href=re.compile(
                    r"https?://[^\"]*ign\.com/.*(?:/review/|/reviews/|/article/)", re.I
                ),
            )
        ...
        for block_match in block_pattern.finditer(html):
            ...
            # match IGN review URLs including localized subdomains (me.ign.com) and /review/ paths
            ext_m = re.search(
                r'href="(https?://[^\"]*ign\.com/[^\"]*(?:/review/|/reviews/|/article/)[^\"]*)"',
                block,
                re.I,
            )
...
# Save to JSON file
with open("ign_games.json", "w", encoding="utf-8") as f:
    json.dump(games_dict, f, indent=2, ensure_ascii=False)

print(f"\nSaved to ign_games.json")
